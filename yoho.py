...
def main():
    ...
    if st.sidebar.button('撮影開始'):
        ...
        final_distance = np.mean(final_distances) if final_distances else None
        ...
        # 三角形の面積の変化率を計算
        final_triangle_areas = []
        if keypoints_list and scores_list:
            for keypoints, scores in zip(keypoints_list, scores_list):
                area = calculate_triangle_area(keypoints, scores)
                if area is not None:
                    final_triangle_areas.append(area)
